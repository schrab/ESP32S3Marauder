...
void Display::displayBuffer(bool do_clear)
{
  ...
  {
    int print_count = 1; // Limit the number of lines to process in one go
    ...
    while ((display_buffer->size() > 0) && (print_count > 0))
    {
        ...
        if ((display_buffer->size() > 0) && (!loading))
        {
          ...
          this->processAndPrintString(tft, display_buffer->shift());
          printing = false;
          delay(print_delay_2);
        }
        if (!tteBar)
        ...
        for (int i = 0; i < this->screen_buffer->size(); i++) {
          ...
          // Use the new color processing function
...
void Display::buildBanner(String msg, int xpos)
{
  ...
  this->tft.fillRect(0, STATUS_BAR_WIDTH, SCREEN_WIDTH, TEXT_HEIGHT + 4, TFT_BLACK);
  ...
  this->showCenterText("[ " + msg + "]", STATUS_BAR_WIDTH + 2);
