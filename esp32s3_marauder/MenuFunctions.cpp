...
if (r_btn.justPressed()) {
    while (!r_btn.justReleased()) {
        r_btn.justPressed(); // Need to continue updating button hold status. My adjustment to the library.
        ...
        delay(1);
            ...
            // if (r_btn.justPressed()) {
            //   while (!r_btn.justReleased()) {
            //     r_btn.justPressed(); // Need to continue updating button hold status. My shitty library.
...
void MenuFunctions::updateStatusBar()
{
  ...
  #if defined(MARAUDER_MINI) || defined(MARAUDER_M5STICKC) || defined(ESP32_S3_MINI)
    display_obj.tft.setFreeFont(NULL);
  #endif
  ...
  if (this->old_gps_sat_count != gps_obj.getNumSats()) {
    ...
    display_obj.tft.fillRect(0, 0, SCREEN_WIDTH / 8, STATUS_BAR_WIDTH, STATUSBAR_COLOR);
    status_changed = true;
  }

    if (gps_obj.getGpsModuleStatus()) {
      if (gps_obj.getFixStatus()) {
          // Get the datetime string from gps_obj.getDatetime()
          String datetimeStr = gps_obj.getDatetime();
          // Parse the datetime string to extract individual values
          int year, month, day, hour, minute, second;
          sscanf(datetimeStr.c_str(), "%d-%d-%d %d:%d:%d", &year, &month, &day, &hour, &minute, &second);
          // Set the RTC clock with the extracted values
          rtc.setTime(second, minute, hour, day, month, year);
          // rtc.setTime(seconds, minutes, hours, day of the month, month, year);
          // Example: rtc.setTime(0, 20, 16, 27, 6, 2024); for 27th Jun 2024 4:20:00
        the_color = TFT_GREEN;
      } else {
        the_color = TFT_RED;
      }
        ...
        display_obj.tft.setTextColor(the_color, STATUSBAR_COLOR);
        display_obj.tft.drawString("GPS " + gps_obj.getNumSatsString(), 0, 1, 1);
      #endif
  ...
  // WiFi Channel Stuff Update
  if ((wifi_scan_obj.set_channel != wifi_scan_obj.old_channel) || (status_changed)) {
    wifi_scan_obj.old_channel = wifi_scan_obj.set_channel;
    #if defined(MARAUDER_MINI) || defined(MARAUDER_M5STICKC) || defined(ESP32_S3_MINI)
      display_obj.tft.fillRect(SCREEN_WIDTH/8, 0, SCREEN_WIDTH / 8, STATUS_BAR_WIDTH, STATUSBAR_COLOR); // STATUSBAR_COLOR
    #else
    ...
    #if defined(MARAUDER_MINI) || defined(MARAUDER_M5STICKC) || defined(ESP32_S3_MINI)
      display_obj.tft.setTextWrap(false);
      display_obj.tft.drawString("CH: " + (String)wifi_scan_obj.set_channel, SCREEN_WIDTH/8, 1, 1);
    #endif
  }
  ...
  if ((wifi_scan_obj.free_ram != wifi_scan_obj.old_free_ram) || (status_changed)) {
    ...
    display_obj.tft.fillRect(SCREEN_WIDTH/1.3, 0, 40, STATUS_BAR_WIDTH, STATUSBAR_COLOR);
    #ifdef HAS_ILI9341
      display_obj.tft.drawString((String)wifi_scan_obj.free_ram + " KB", 100, 0, 2);
    #endif
    
    #if defined(ESP32_S3_MINI) || defined(MARAUDER_MINI) || defined(MARAUDER_M5STICKC) || defined(MARAUDER_REV_FEATHER)
      display_obj.tft.setTextWrap(false);
      display_obj.tft.drawString((String)wifi_scan_obj.free_ram + " Kb", SCREEN_WIDTH/1.3, 1, 1);
    #endif
  }
  ...
  if ((date_time != old_date_time)) {
    old_date_time = date_time;
    display_obj.tft.fillRect(SCREEN_WIDTH/2.4, 0, 60, STATUS_BAR_WIDTH, STATUSBAR_COLOR);
    #if defined(ESP32_S3_MINI) || defined(MARAUDER_MINI) || defined(MARAUDER_M5STICKC) || defined(MARAUDER_REV_FEATHER)
      display_obj.tft.drawString((String)date_time, SCREEN_WIDTH/2.4, 1, 1);
    #endif
  }
...
void MenuFunctions::drawStatusBar()
{
  ...
  display_obj.tft.setTextWrap(false);
  display_obj.tft.setTextColor(TFT_WHITE, STATUSBAR_COLOR); // STATUSBAR_COLOR
  ...
  // WiFi Channel Stuff Draw
  wifi_scan_obj.old_channel = wifi_scan_obj.set_channel;
  #if defined(MARAUDER_MINI) || defined(MARAUDER_M5STICKC) || defined(ESP32_S3_MINI)
    display_obj.tft.fillRect(SCREEN_WIDTH/8, 0, SCREEN_WIDTH / 8, STATUS_BAR_WIDTH, STATUSBAR_COLOR);
  #else
    display_obj.tft.fillRect(50, 0, SCREEN_WIDTH * 0.21, STATUS_BAR_WIDTH, STATUSBAR_COLOR);
  #endif
  #ifdef HAS_ILI9341
...
void MenuFunctions::RunSetup()
{
  ...
  this->addNodes(&wifiSnifferMenu, text_table1[49], TFT_MAGENTA, NULL, BEACON_SNIFF, [this]() {
    ...
    this->drawStatusBar();
    wifi_scan_obj.StartScan(WIFI_SCAN_TARGET_AP, TFT_MAGENTA); // Scan AP's
  }); 
  // #ifdef HAS_ILI9341
    this->addNodes(&wifiSnifferMenu, "Signal Monitor", TFT_CYAN, NULL, PACKET_MONITOR, [this]() {
      display_obj.clearScreen();
      this->drawStatusBar();
      wifi_scan_obj.StartScan(WIFI_SCAN_SIG_STREN, TFT_CYAN);  // Signal Monitor
    });
  // #endif
  #ifdef HAS_GPS
    if (gps_obj.getGpsModuleStatus()) {
      this->addNodes(&wifiSnifferMenu, "Wardrive", TFT_GREEN, NULL, BEACON_SNIFF, [this]() {
        display_obj.clearScreen();
        this->drawStatusBar();
        wifi_scan_obj.StartScan(WIFI_SCAN_WAR_DRIVE, TFT_GREEN); // Wardrive
      });
    }
  #endif
  // #ifdef HAS_GPS
  ...
  // #endif
    ...
    this->addNodes(&wifiSnifferMenu, text_table1[46], TFT_VIOLET, NULL, EAPOL, [this]() {
      display_obj.clearScreen();
      this->drawStatusBar();
      wifi_scan_obj.StartScan(WIFI_SCAN_EAPOL, TFT_VIOLET); // EAPOL/PMKID Scan
    });
    // this->addNodes(&wifiSnifferMenu, "Active EAPOL/PMKID Scan", TFT_VIOLET, NULL, EAPOL, [this]() {
    //   display_obj.clearScreen();
    //   this->drawStatusBar();
    //   wifi_scan_obj.StartScan(WIFI_SCAN_ACTIVE_EAPOL, TFT_ORANGE); // Deauth EAPOL/PMKID Scan
    // });
    //  this->addNodes(&wifiSnifferMenu, "Active List EAPOL/PMKID Scan", TFT_VIOLET, NULL, EAPOL, [this]() {
    //   display_obj.clearScreen();
    //   this->drawStatusBar();
    //   wifi_scan_obj.StartScan(WIFI_SCAN_ACTIVE_LIST_EAPOL, TFT_BROWN); // Deauth List EAPOL/PMKID Scan
    // });
    
    this->addNodes(&wifiSnifferMenu, text_table1[45], TFT_BLUE, NULL, PACKET_MONITOR, [this]() {
      display_obj.clearScreen();
      this->drawStatusBar();
      wifi_scan_obj.StartScan(WIFI_PACKET_MONITOR, TFT_BLUE); // Packet Monitor
    });
    ...
    this->addNodes(&wifiSnifferMenu, text_table1[47], TFT_RED, NULL, PWNAGOTCHI, [this]() {
      display_obj.clearScreen();
      this->drawStatusBar();
      wifi_scan_obj.StartScan(WIFI_SCAN_PWN, TFT_RED); // Detect Pwnagotchi
    });
  #endif
  ...
  this->addNodes(&wifiSnifferMenu, text_table1[59], TFT_ORANGE, NULL, PACKET_MONITOR, [this]() {
    display_obj.clearScreen();
    this->drawStatusBar();
    wifi_scan_obj.StartScan(WIFI_SCAN_STATION, TFT_WHITE); // Station Sniff
  });
  this->addNodes(&wifiSnifferMenu, text_table1[58], TFT_WHITE, NULL, PACKET_MONITOR, [this]() {
    display_obj.clearScreen();
    this->drawStatusBar();
    wifi_scan_obj.StartScan(WIFI_SCAN_RAW_CAPTURE, TFT_WHITE); // Raw Capture
  });
 
  this->addNodes(&wifiSnifferMenu, text_table1[42], TFT_CYAN, NULL, PROBE_SNIFF, [this]() {
    display_obj.clearScreen();
    this->drawStatusBar();
    wifi_scan_obj.StartScan(WIFI_SCAN_PROBE, TFT_CYAN); // Probe request Sniff
  });
  this->addNodes(&wifiSnifferMenu, text_table1[43], TFT_MAGENTA, NULL, BEACON_SNIFF, [this]() {
    display_obj.clearScreen();
    this->drawStatusBar();
    wifi_scan_obj.StartScan(WIFI_SCAN_AP, TFT_MAGENTA); // Beacon Sniff
  });
  this->addNodes(&wifiSnifferMenu, text_table1[44], TFT_RED, NULL, DEAUTH_SNIFF, [this]() {
    display_obj.clearScreen();
    this->drawStatusBar();
    wifi_scan_obj.StartScan(WIFI_SCAN_DEAUTH, TFT_RED); // Deauth Sniff
  });
  ...
  this->addNodes(&wifiAttackMenu, text_table1[50], TFT_RED, NULL, BEACON_LIST, [this]() {
    display_obj.clearScreen();
    this->drawStatusBar();
    wifi_scan_obj.StartScan(WIFI_ATTACK_BEACON_LIST, TFT_RED); // Beacon Spam List
  });
  this->addNodes(&wifiAttackMenu, text_table1[51], TFT_ORANGE, NULL, BEACON_SPAM, [this]() {
    display_obj.clearScreen();
    this->drawStatusBar();
    wifi_scan_obj.StartScan(WIFI_ATTACK_BEACON_SPAM, TFT_ORANGE); // Beacon Spam Random
  });
  this->addNodes(&wifiAttackMenu, text_table1[52], TFT_YELLOW, NULL, RICK_ROLL, [this]() {
    display_obj.clearScreen();
    this->drawStatusBar();
    wifi_scan_obj.StartScan(WIFI_ATTACK_RICK_ROLL, TFT_YELLOW); // Rick Roll Beacon
  });
  this->addNodes(&wifiAttackMenu, text_table1[53], TFT_RED, NULL, PROBE_SNIFF, [this]() {
    display_obj.clearScreen();
    this->drawStatusBar();
    wifi_scan_obj.StartScan(WIFI_ATTACK_AUTH, TFT_RED); // Probe Req Flood
  });
  ...
  this->addNodes(&wifiAttackMenu, text_table1[54], TFT_RED, NULL, DEAUTH_SNIFF, [this]() {
    display_obj.clearScreen();
    this->drawStatusBar();
    wifi_scan_obj.StartScan(WIFI_ATTACK_DEAUTH, TFT_RED); // Deauth Flood
  });
  this->addNodes(&wifiAttackMenu, text_table1[57], TFT_MAGENTA, NULL, BEACON_LIST, [this]() {
    display_obj.clearScreen();
    this->drawStatusBar();
    wifi_scan_obj.StartScan(WIFI_ATTACK_AP_SPAM, TFT_MAGENTA); // AP Clone Spam
  });
  this->addNodes(&wifiAttackMenu, text_table1[62], TFT_RED, NULL, DEAUTH_SNIFF, [this]() {
    display_obj.clearScreen();
    this->drawStatusBar();
    wifi_scan_obj.StartScan(WIFI_ATTACK_DEAUTH_TARGETED, TFT_ORANGE);  // Deauth Targeted
  });
  ...
  for (int i = 0; i < settings_obj.getNumberSettings(); i++) {
    if (this->callSetting(settings_obj.setting_index_to_name(i)) == "bool")
      this->addNodes(&settingsMenu, settings_obj.setting_index_to_name(i), TFT_GREEN, NULL, 0, [this, i]() {
